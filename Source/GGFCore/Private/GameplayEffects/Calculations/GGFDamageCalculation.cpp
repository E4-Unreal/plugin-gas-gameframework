    ...
    UAbilitySystemComponent* SourceSystem, UAbilitySystemComponent* TargetSystem) const
{
    ...
    // 스탯 데미지 계산
    GenericDamage = FMath::Max(0, GenericDamage - TargetDefense);

    // 총 데미지 계산
    TotalDamage = GenericDamage + TrueDamage;
...
#if WITH_EDITOR
    LOG(Log, TEXT("%s Take Damage From %s: TotalDamage(%f) = (GenericDamage(%f) + TrueDamage(%f)) * DamageRatio(%f)"), *TargetSystem->GetAvatarActor()->GetName(), *SourceSystem->GetAvatarActor()->GetName(), TotalDamage, GenericDamage, TrueDamage, DamageRatio)
#endif
